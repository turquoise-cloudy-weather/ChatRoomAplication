...
def receive():
    while True:
        try:
            msg = client_socket.receive(1024).decode("utf8")
...
label = Label(window, text="Enter message: ", fg="blue", font='Aeria', bg="Red")
label.pack()

entry_feed = Entry(window, textvariable=my_msg, fg="red", width=50)
entry_feed.pack()

send_Button =  Button(window, text = "Send", font = "Aerial", fg='White', command=send)
send_Button.pack()

quit_Button = Button(window, texy = "Quit", font = "Aerial", fg = "White", command = on_closing)
quit_Button.pack()
